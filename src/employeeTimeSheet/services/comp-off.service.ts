...
export class CompOffService {
  ...
  async markAsPending(
    ...
    for (let i = 0; i < creditDates.length; i++) {
      ...
      if (compOff) {
        ...
        takenDatesObj[leaveDate] =
          durationPerDay === 0.5 ? halfDayLabel : 'Full';
        compOff.takenDates = JSON.stringify(takenDatesObj);

        compOff.remainingDays = Number(
          (compOff.remainingDays - durationPerDay).toFixed(1),
        );
        ...
        this.logger.warn(
          `[MARK_PENDING] Credit record NOT FOUND for date: ${creditDate}`,
        );
      }
    }
  }
  ...
  async restoreCompOffs(leaveRequestId: number) {
    ...
    for (const compOff of records) {
      if (!compOff.takenDates) {
        ...
        compOff.status =
          compOff.remainingDays >= 1.0
            ? CompOffStatus.NOT_TAKEN
            : CompOffStatus.HALF_TAKEN;
        await this.compOffRepository.save(compOff);
        continue;
      }
      ...
      try {
        ...
        compOff.remainingDays = Number(
          (compOff.remainingDays + restoredAmount).toFixed(1),
        );
  ...
  async getCompOffHistory(
    ...
    const query = this.compOffRepository.createQueryBuilder('compOff')
    ...
    if (status && status !== 'All Status' && status !== 'All') {
      ...
      if (status === 'NOT_TAKEN' || status === 'Not Taken') {
        statusValue = CompOffStatus.NOT_TAKEN;
      } else if (status === 'FULL_TAKEN' || status === 'Full Taken') {
        statusValue = CompOffStatus.FULL_TAKEN;
      } else if (status === 'HALF_TAKEN' || status === 'Half Taken') {
        statusValue = CompOffStatus.HALF_TAKEN;
      } else if (status === 'PENDING' || status === 'Pending') {
        statusValue = CompOffStatus.PENDING;
      }
